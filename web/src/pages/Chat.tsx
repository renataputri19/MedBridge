...
export default function Chat() {
  ...
  /*
   * A submitted request has nothing left to edit, so the column comes off and
   * the conversation has the page back — which is where the confirmation
   * belongs, at the end of the conversation that produced it.
   */
  ...
  useEffect(() => {
    ...
    // the transcript, and both should leave it sitting at the bottom. `hasPlan`
    // is in here because the card is re-measured when the plan appears beside
    // it: on a phone it shortens to make room, which moves the bottom.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lastMessage?.id, lastMessage?.ui?.kind, session?.messages.length, pending, thinking, hasPlan])
